...
void ledTempBar(float temp){ 
   ...
   for(int i=255; i>= 0; i= i-20){
    ...
    currMills = millis();
    analogWrite(pwmPin, 255);
    if ((currMills - prevMills) >= errorIndicatorDelay){
      toggle = !toggle;
      prevMills = currMills; 
    }
    if(toggle) for(int j=0; j<=9 ; j++) digitalWrite(j + led0, HIGH);           // turning on all LEDs at once
    else for(int j=0; j<=9 ; j++) digitalWrite(j + led0, LOW);                  // turning off all LEDs at once
  }
}
